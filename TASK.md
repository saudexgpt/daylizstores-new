...
- [x] **Live-server fix: slow checkout, duplicate orders, missing emails** (20 Sep). Cause: the customer's login email was sent inline INSIDE the order transaction, so an unreachable SMTP server held the request (and the stock locks) open past the 30-second duplicate-order cache lock; the customer clicked again and a second order got in — and neither email was ever sent. Fixed:
  - one checkout attempt = one order, enforced by the database: unique index `orders_order_uniq_id_unique` (migration `2026_09_22_100000`); a racing retry now gets "order already placed" with the existing order instead of a second one. The migration also neutralised **24 historical duplicate orders (23 groups, since 2024)** — it clears only the technical id on the later copy and writes the pairs to `storage/app/duplicate-orders-<time>.csv` to review (4 pairs have both copies paid)
  - all three emails (order details, new-customer login, password reset) are now QUEUED jobs with 5 retries and back-off, dispatched only after the order commits and after the response; the login details and the reset code are encrypted in the queue table
  - mail server timeout 15 s (`MAIL_TIMEOUT`), Paystack 5 s connect / 15 s read; checkout Submit / Pay buttons disabled while sending, and a timeout message tells the customer pressing again is safe
  - **Production needs**: `QUEUE_CONNECTION=database`, the queue worker running (`deploy/daylizstores-queue.service`), `queue:restart` in the deploy script, and outbound SMTP reachable from the droplet (DigitalOcean commonly blocks ports 25 / 465 / 587 — test with `nc -vz -w5 <smtp host> <port>`; Mailtrap also offers 2525). Failed emails: `php artisan queue:failed` / `queue:retry all`
